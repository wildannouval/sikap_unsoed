                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Jadwal Seminar (Final)</th>
                            <th scope="col" class="px-6 py-3">Ruangan (Final)</th>
                            <th scope="col" class="px-6 py-3">Nilai</th>
                            <th scope="col" class="px-6 py-3">Catatan Hasil</th>
                        </tr>
                        </thead>
                        <tbody>
                        @forelse ($seminars as $seminar)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $loop->iteration + $seminars->firstItem() - 1 }}</td>
                                ...
                                <td class="px-6 py-4">
                                    @if($seminar->status_pengajuan == 'diajukan_mahasiswa')
                                        <span class="bg-yellow-100 text-yellow-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">Diajukan (Menunggu Dospem)</span>
                                    ...
                                    @elseif($seminar->status_pengajuan == 'ditolak_dospem')
                                        ...
                                        @if($seminar->catatan_dospem)
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 italic">Catatan: {{ Str::limit($seminar->catatan_dospem, 50) }}</p>
                                        @endif
                                    ...
                                    @elseif($seminar->status_pengajuan == 'revisi_jadwal_bapendik')
                                        ...
                                        @if($seminar->catatan_komisi)
                                            <p class="text-xs text-gray-500 mt-1 italic">Catatan: {{ Str::limit($seminar->catatan_komisi, 50) }}</p>
                                        @endif
                                ...
                                <td class="px-6 py-4">
                                    @if($seminar->status_pengajuan == 'dijadwalkan_komisi' || $seminar->status_pengajuan == 'selesai_dinilai')
                                        {{ $seminar->tanggal_seminar ? \Carbon\Carbon::parse($seminar->tanggal_seminar)->isoFormat('dddd, D MMM YY') : '' }}
                                        <br> <span class="text-xs text-gray-500 dark:text-gray-400">{{ $seminar->jam_mulai ? \Carbon\Carbon::parse($seminar->jam_mulai)->format('H:i') : '' }}{{ $seminar->jam_selesai ? ' - ' . \Carbon\Carbon::parse($seminar->jam_selesai)->format('H:i') : '' }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-6 py-4">{{ ($seminar->status_pengajuan == 'dijadwalkan_komisi' || $seminar->status_pengajuan == 'selesai_dinilai') ? ($seminar->ruangan ?? '-') : '-' }}</td>
                                <td class="px-6 py-4">
                                    @if($seminar->status_pengajuan == 'selesai_dinilai')
                                        ...
                                        @else
                                            <span class="text-xs text-yellow-600 dark:text-yellow-400 italic">Upload bukti distribusi untuk lihat nilai.</span>
                                        @endif
                                    @else
                                        -
                                    @endif
