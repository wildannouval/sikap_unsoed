        ...
        <div class="mx-auto max-w-none px-4 lg:px-6 py-4">
            <div class="bg-white dark:bg-gray-800 relative shadow-xl sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 border-b dark:border-gray-700">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Data Jurusan</h2>
                    <a href="{{ route('bapendik.jurusan.create') }}" class="flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                ...
                <div class="px-4 pt-4"> {{-- Bungkus partial agar paddingnya konsisten --}}
                    @include('partials.session-messages')
                </div>

                {{-- Form Search --}}
                <form method="GET" action="{{ route('bapendik.jurusan.index') }}">
                    <div class="flex flex-col md:flex-row items-end space-y-3 md:space-y-0 md:space-x-4 p-4">
                        <div class="w-full md:w-1/2">
                            <label for="search_jurusan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cari Jurusan</label>
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="search_jurusan" value="{{ $request->search ?? '' }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Cari kode, nama, fakultas...">
                            </div>
                        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        {{-- ... (thead dan tbody tabel jurusan seperti sebelumnya) ... --}}
                        ...
                        @forelse ($jurusans as $jurusan)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $loop->iteration + $jurusans->firstItem() - 1 }}
                                </th>
                                <td class="px-6 py-4">{{ $jurusan->kode }}</td>
                                <td class="px-6 py-4">{{ $jurusan->nama }}</td>
                                <td class="px-6 py-4">{{ $jurusan->fakultas }}</td>
                                <td class="px-6 py-4">{{ $jurusan->jenjang }}</td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('bapendik.jurusan.edit', $jurusan->id) }}" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-xs px-3 py-1.5 mr-2 dark:bg-yellow-600 dark:hover:bg-yellow-700 focus:outline-none dark:focus:ring-yellow-800">
                                        <svg class="w-4 h-4 inline-block mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd"></path></svg>
                                        Edit
                                    </a>
                                    <button type="button"
...
@push('scripts')
    {{-- Script untuk modal delete dan sukses (jika pakai modal) sama seperti sebelumnya --}}
    <script>
