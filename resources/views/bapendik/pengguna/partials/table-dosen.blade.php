{{-- Form Search dan Filter untuk Dosen --}}
<form method="GET" action="{{ route('bapendik.pengguna.index') }}">
    <input type="hidden" name="dosen_page" value="{{ $request->input('dosen_page', 1) }}">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-white dark:bg-gray-900 rounded-lg shadow">
        <div>
            <label for="search_dosen" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cari Dosen (Nama, Email, NIDN)</label>
            <input type="text" name="search_dosen" id="search_dosen" value="{{ $request->search_dosen }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ketik kata kunci...">
        </div>
        <div>
            <label for="jurusan_dosen" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Filter Jurusan</label>
            <select name="jurusan_dosen" id="jurusan_dosen" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="">Semua Jurusan</option>
                @foreach ($jurusans as $jurusan)
                    <option value="{{ $jurusan->id }}" {{ $request->jurusan_dosen == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama }}</option>
                @endforeach
            </select>
        </div>
...
<div class="overflow-x-auto">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
        @forelse ($users as $user) {{-- users di sini adalah $dosens dari controller --}}
        <tr class="border-b dark:border-gray-700">
            ...
            <td class="px-4 py-3 flex items-center justify-end">
                <button id="dosen-dropdown-button-{{$user->id}}" data-dropdown-toggle="dosen-dropdown-{{$user->id}}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 rounded-lg" type="button">
                ...
                <div id="dosen-dropdown-{{$user->id}}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow">
        ...
        @empty
            <tr><td colspan="5" class="px-4 py-3 text-center">Data dosen tidak ditemukan.</td></tr>
        @endforelse
        </tbody>
    </table>
</div>
<nav class="p-4">
    {{ $users->appends(request()->except('dosen_page'))->links() }}
</nav>
